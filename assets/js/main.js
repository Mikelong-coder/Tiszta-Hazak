...
/* Süti stub — teljes UI: cookie-consent.js (idle / interakció után) */
...
function applyConsentClasses(consent) {
  if (!consent) return;
...
function cookieScriptUrl() {
  const main = document.querySelector('script[src*="main.js"]');
  const m = main && main.src.match(/[?&]v=(\d+)/);
  const v = (m && m[1]) || '1';
  return 'assets/js/cookie-consent.js?v=' + v;
}

function loadCookieModule() {
  if (window.THCookie) return Promise.resolve(window.THCookie);
  if (window.__thCookieLoading) return window.__thCookieLoading;
  window.__thCookieLoading = new Promise((resolve, reject) => {
    const s = document.createElement('script');
    s.src = cookieScriptUrl();
    s.async = true;
    s.onload = () => resolve(window.THCookie);
    s.onerror = () => reject(new Error('cookie-consent load failed'));
    document.head.appendChild(s);
  });
  return window.__thCookieLoading;
}

function openCookieSettings(opts) {
  loadCookieModule()
    .then((mod) => mod.openSettings(opts || {}))
    .catch(() => {});
}
...
function initCookieConsent() {
  ...
  if (existing) {
    applyConsentClasses(existing);
    if (existing.marketing) {
      scheduleIdle(() => {
        loadCookieModule()
          .then((mod) => mod.applyConsent(existing))
          .catch(() => {});
      }, 2500);
    }
  }
