...
/* ── Süti (cookie) hozzájárulás ───────────────────────────────────────────
   Szükséges: mindig · Funkcionális: Google Maps · Marketing: Google Ads
   Süti neve: cookie_consent (1 év) — lásd cookie.html
   Google Ads ID: <html data-google-ads-id="AW-…"> vagy GOOGLE_ADS_ID alább. */
const COOKIE_CONSENT_NAME = 'cookie_consent';
const COOKIE_CONSENT_DAYS = 365;
const COOKIE_CONSENT_VERSION = 1;
/** Üresen hagyható — ha megvan az Ads konverzióazonosító, ide vagy data-google-ads-id-re. */
const GOOGLE_ADS_ID = '';
...
function loadMapHost(host) {
  ...
  iframe.className = 'contact-block__map-frame';
...
function setupMapFacades() {
  ...
  document.querySelectorAll('[data-map-facade]').forEach((host) => {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!hasFunctionalConsent()) {
        openCookieSettings({ highlight: 'functional', pendingMap: host });
        return;
      }
      loadMapHost(host);
    });
  ...
  document.addEventListener('cookieconsent:change', (event) => {
    if (!event.detail?.functional) return;
    const pending = document.querySelector('[data-map-facade][data-map-pending]');
    if (pending) {
      pending.removeAttribute('data-map-pending');
      loadMapHost(pending);
    }
  });
}
...
function buildCookieUi() {
  ...
  const closeModal = () => {
    ...
    if (cookieLastFocus && typeof cookieLastFocus.focus === 'function') {
  ...
  const openModal = (opts = {}) => {
    ...
    hideBanner();
    window.setTimeout(() => {
      (opts.highlight === 'functional' ? functionalToggle : root.querySelector('[data-cookie-close].cookie-modal__x'))?.focus();
    }, 30);
  };
  ...
  const finish = (consent) => {
    ...
    hideBanner();
    closeModal();
    if (consent.functional && cookiePendingMap) {
      loadMapHost(cookiePendingMap);
      cookiePendingMap.removeAttribute('data-map-pending');
      cookiePendingMap = null;
    } else if (!consent.functional && cookiePendingMap) {
      cookiePendingMap.removeAttribute('data-map-pending');
      cookiePendingMap = null;
    }
  };
  ...
  root.addEventListener('click', (e) => {
    const t = e.target.closest('[data-cookie-accept-all], [data-cookie-reject], [data-cookie-customize], [data-cookie-save], [data-cookie-close]');
    if (!t) return;
